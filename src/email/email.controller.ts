...
import {
  EmailSignInResponse,
  EmailSignUpResponse,
} from '@email/email-response-examples';
...
export class EmailController {
  ...
  async emailSignUp(@Body() body: any): Promise<UserSignUpResponse> {
    const { email, password } = body;
    const response = await this._emailService.emailSignUp(email, password);
    return response;
  }
  ...
  async emailSignIn(@Body() body: any): Promise<any> {
    const { email, password } = body;
    const response = await this._emailService.emailSignIn(email, password);
    return response;
  }
  ...
  async emailResetPassword(@Body() body: any): Promise<any> {
    const { email } = body;
    const response = await this._emailService.emailResetPassword(email);
    return response;
  }
}
